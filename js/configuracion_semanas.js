...
function getConfiguracionSemanas() {
  ...
  $.blockUI({
    message: "<h4> TRAYENDO SEMANAS...</h4>",
      ...
      if (response.status == "success") {
        ...
        for (var i = 0; i < response.data.length; i++) {
          ...
          table_semanas.row.add([
            ...
            status,
            `                                                                                            
                        <button class="btn btn-warning btn_editar_semana" onclick="modalEditarSemana(this, ${
                          response.data[i].id
                        }, ${response.data[i].cantidad}, ${
              response.data[i].interes
            }, ${response.data[i].tipo_abono}, ${
              response.data[i].semana_renovacion
            })" title="Editar semana" data-toggle="modal" data-target="#modal_editar_semana"><i class="fa-solid fa-pen-to-square" ></i></button>
                         ${
                           response.data[i].status == 1
                             ? `<button class="btn btn-danger btn_desactivar_semana" onclick="desactivarSemana( ${response.data[i].id})" title="Desactivar semana"><i class="fa-solid fa-ban" ></i></button>`
...
btn_guardar_semana.click(function () {
  if (
    ...
    inp_semana_cantidad.val() == "0" ||
    inp_semana_interes.val() == "0" ||
    inp_semana_semana_ren.val() == "0"
  ) {
    Swal.fire({
      icon: "warning",
      title: "Campos vacios",
...
btn_guardar_editar_semana.click(function () {
  if (
    inp_semana_cantidad_editar.val() == "" ||
    inp_semana_interes_editar.val() == "" ||
    inp_semana_semana_ren_editar.val() == "" ||
    inp_semana_cantidad_editar.val() == "0" ||
    inp_semana_interes_editar.val() == "0" ||
    inp_semana_semana_ren_editar.val() == "0"
  ) {
    Swal.fire({
      icon: "warning",
      title: "Campos vacios",
