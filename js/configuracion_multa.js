...
function getConfiguracionMulta(){
    ...
    $.blockUI({ message: '<h4> TRAYENDO MULTA...</h4>', css: { backgroundColor: null, color: '#fff', border: null } });
            ...
            if(response.status == 'success'){
                ...
                for(var i = 0; i < response.data.length; i++ ){
                    ...
                    tabla_multa.row.add([
                        ...
                        status,
                        `                                                                                            
                        <button class="btn btn-warning btn_editar_multa" onclick="modalEditarMulta(this, ${response.data[i].id}, ${response.data[i].cantidad})" title="Editar multa" data-toggle="modal" data-target="#modal_editar_multa"><i class="fa-solid fa-pen-to-square" ></i></button>
                         ${ response.data[i].status == 1 ? `<button class="btn btn-danger btn_desactivar_multa" onclick="desactivarMulta( ${response.data[i].id})" title="Desactivar multa"><i class="fa-solid fa-ban" ></i></button>`
